...
OS_Excel = 'C:/My Files/Python_Projects/Python Tutorials/Python Stuff/On-Going Projects/Data Entry GUI/User_gadget_Tracking.xlsx'
...
layout = [
          ...
          [sg.Text('Please input ID: ',size = (35,1)), sg.InputText(key = 'ID')], 
          [sg.Text('Please Input Current Shift: ', size = (35,1)), sg.Combo(['A2A1', 'C2B1','A1D1', 'D1B2', 'B1B2', 'C1A2', 'C1C2'], key = 'Shift', readonly = True)],  
          
          [sg.Text('')],
    
          [sg.Text('gadget Details: ', text_color = 'red', font = 15)],
    
          [sg.Text('Link: ', size = (35,1)), sg.InputText(key = 'Link')],
          [sg.Text('Machine & Valve Location', size = (35,1)), sg.Combo(['D1V1', 'D1V2', 'D2V1', 'D2V2', 'D3V1', 'D3V2'],key = 'Machine & Valve Location', readonly = True)],
          [sg.Text('gadget ID Returned', size = (35,1)), sg.InputText(key = 'gadget_Returned')],  ## Not defining anything inside sg.InputText() defaults the value index as [0], if we specify a key, the value will be stored as the new key         
         ## Since I can select more than one checkbox, consider radio buttons/Combobox. REFERENCE: https://holypython.com/gui-with-python-checkboxes-and-radio-buttons-pysimplegui-part-ii/
          [sg.Text('Returned gadget Type', size = (35,1)), sg.Combo(['A', 'B', 'C', 'D' ], key = 'Returned gadget Type',readonly = True)], 

         [sg.Text('gadget ID Taken', size = (35,1)), sg.InputText(key = 'gadget_Taken')],                                 
...
window = sg.Window('gadget Tracking GUI', layout)
